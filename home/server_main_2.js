    ...
    <body>
        <fieldset>
            ...
            <form action="/data" method="post" id="dataForm">
                ...
                <input class="Title" type="text" id="Title" name="Title" /><br />
                <label for="content">게시글내용 </label>
                <input class="content" type="text" id="content" name="content" /><br />
                ...
                <button type="submit" onclick="alert('작성이 완료되었습니다.');location.href='/list'">작성</button>
...
app.get('/list', (req, res) => {
    ...
    db.query(`SELECT Num, Title, Name, Today, Count FROM home`, (err, results) => {
        ...
        list += `            <button type="button" onclick="location.href='/content'">게시글작성</button>`;
        list += `        </div>`;
        list += `        <table>`;
        list += `            <tr>`;
        if (req.session && req.session.username == 'admin') {
